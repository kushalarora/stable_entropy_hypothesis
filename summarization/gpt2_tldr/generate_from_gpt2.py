...
def main():
    ...
    dataset = load_dataset("CarperAI/openai_summarize_tldr", cache_dir="/home/mila/a/arorakus/scratch/mpror_rl/trlx/data")
    ...
    def tokenizer_method(examples):
        tokenized_examples = tokenizer(examples['prompt'], max_length=612, truncation=True, padding=True, return_tensors='pt')
        return tokenized_examples, examples['label']
    ...
    with open(args.output_filename, 'w') as output_file, \
        ...
        prompt_sequences = []
        targets = []
        for idx, batch_start in enumerate(trange(0, len(test_set), args.batch_size)):
            batch, batch_targets = tokenizer_method(test_set[batch_start: batch_start+args.batch_size])
