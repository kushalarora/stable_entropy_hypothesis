...
def main():
    ...
    xsum_dataset = load_dataset("cnn_dailymail", "3.0.0")
    ...
    tokenized_xsum_testset = xsum_testset.map(
        preprocess_function,
        batched=True,
        num_proc=10,
        remove_columns=xsum_dataset['test'].column_names,
    ...
    test_dataloader = DataLoader(tokenized_xsum_testset, collate_fn=data_collator, batch_size=args.batch_size)
    ...
    with open(args.output_filename, 'w') as output_file:
        ...
        for idx, batch in enumerate(test_dataloader):
            ...
            documents = tokenizer.batch_decode(batch['input_ids'],  skip_special_tokens=True)

            for generated_sequence_idx, (document, generated_summary) \
                    in enumerate(zip(documents, generated_summaries)):
                ...
                print(f"{document}\t{generated_summary}", file=output_file, flush=True)
