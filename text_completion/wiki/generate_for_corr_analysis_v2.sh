...
mkdir -p data/text_completion/corr_analysis/gpt2_xl/

# Greedy
filename=data/text_completion/corr_analysis/gpt2_xl/greedy.jsonl
...
do 
    filename=data/text_completion/corr_analysis/gpt2_xl/beam_${num_beams}.jsonl
    if [[ ! -f "${filename}" ]] || [[ $(($(wc -l < ${filename}) < 5000)) -eq 1 ]];
    then
...
for run in 1 2 3 4 5; do
    ...
    # for p in 0.15 0.4; 
    do
        filename="data/text_completion/corr_analysis/gpt2_xl/top_p_${p}_run_${run}.jsonl"
        seed=$((1 + RANDOM % 1000))
        if [ ! -f "${filename}" ] || [ $(($(wc -l < "${filename}") < 5000)) -eq 1 ];
        then
...
# done
    ...
    do
        filename="data/text_completion/corr_analysis/gpt2_xl/top_k_${k}_run_${run}.jsonl"
        seed=$((1 + RANDOM % 1000))
        if [ ! -f "${filename}" ] || [ $(($(wc -l < "${filename}") < 5000)) -eq 1 ];
        then
    ...
    do
        filename="data/text_completion/corr_analysis/gpt2_xl/typical_p_${tau}_run_${run}.jsonl"
        seed=$((1 + RANDOM % 1000))
        if [ ! -f "${filename}" ] || [ $(($(wc -l < "${filename}") < 5000)) -eq 1 ];
        then
    ...
    do
        filename="data/text_completion/corr_analysis/gpt2_xl/temp_${temp}_run_${run}.jsonl"
        seed=$((1 + RANDOM % 1000))
        if [ ! -f "${filename}" ] || [ $(($(wc -l < "${filename}") < 5000)) -eq 1 ];
        then
