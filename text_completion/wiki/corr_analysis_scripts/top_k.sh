...
for run in 1 2 3 4 5; do
    for k in 5 10 25 50 75 100 500 1000; do
        filename="data/text_completion/corr_analysis/${model_name}/top_k_${k}_run_${run}.jsonl"
        seed=$((1 + RANDOM % 1000))
        if [ ! -f "${filename}" ] || [ $(($(wc -l < "${filename}") < 5000)) -eq 1 ];
        then
            echo "Running Nucleus Sampling: run=${run} || top-p=${p}."
            torchrun --nproc-per-node 8 generate_samples.py --dataset_name wikipedia --model_name_or_path ${model_name} --output_filename ${filename} --bf16 --batch_size ${batch_size} --max_length ${max_length} --max_prompt_length ${max_prompt_length} --num_examples ${num_examples} --k ${k} --do_sample --seed ${seed}
        fi
    done
